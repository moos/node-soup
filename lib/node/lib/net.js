...
function initStream (self) {
  ...
  self._readWatcher.callback = function () {
    ...
    console.log('------- fd,bytesRead', self.fd, bytesRead, self.readable, self.writable)
...
Stream.prototype._writeOut = function (data, encoding, fd) {
  if (!this.writable) {
    throw new Error('Stream is not writable' + this.fd);
  }
  ...
  if (typeof data != 'string') {
    ...
    debug(this.fd + 'wrote ' + bytesWritten + ' bytes to pool' + data);
