...
//				if (fd == 8) debugger;
				...
				if (fd in Sockets) {
					...
					if (!handle.pair || !(handle.pair in Sockets)) {
						...
						defer(function(){ console.log(4444444, fd, peer.readable, peer.fd);  peer.readable = true });
						// writable ??????
				...
				if (fd in Sockets){
					...
					if (how == 'write' || how == 'readwrite') {
						...
						if (!handle.shutdown.read){	// trigger reader
							var h = handle;
//							h.readable = true;
							setTimeout(function(){ console.log(555555, h.readable, h.fd); h.readable = true },1);
						}
					}
...
//		else timer = false;
		...
		next = 0;
		// note: watchers.length may change within loop!
