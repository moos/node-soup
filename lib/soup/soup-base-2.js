...
//				if (fd == 8) debugger;
			...
			write : write = function(fd, buffer, offset, length){
				...
				// write immediately!!!
				watchers.some(function(w){
					
					console.info('!!!!!', watchers.length, w.__fd, pair.fd);
					
					if (!w.__isReadable || w.__fd !== pair.fd) return;	// continue

					// check for shutdown?
					
	 				defer(bindTo(w,w.callback), pair.readable, pair.writable);
	
//					w.callback.call(w, w.__isReadable, w.__isWritable);
				...
				if (enable) {
					...
					if (! handle.keepalive) {
						...
						handle.keepalive.callback = function(){
							...
							console.log('----------- ', fd, handle, handle.fd, handle.readable);
...
//		watchers.forEach(function(w, next){			//////////////////// <<<<< or one at a time!!!!!
			...
			if (1) console.log(next-1,'--- watching w, fd, havedata: ',  fd
...
//		 })
	...
	return function IOWatcher(){
		...
		this.start = function(){
			...
			watchers.push(this);
			if (!timer)
				timer = setInterval(poll, 0);	// at next tick!
