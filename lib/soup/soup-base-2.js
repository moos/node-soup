...
function fd_handle(path,flags,mode){
	...
	this.fd = NEXT_FD++;
	this.eof = false;
//	this.watcher = new process.IOWatcher();
//	this.watcher.set(this.fd,true,true);
//	this.watcher.start();
	//console.log('------ new FD ', this.fd, path)
	Handles[this.fd] = this;
}
...
//				if (fd == 8) debugger;
			...
			setKeepAlive: function(fd,enable,time){
				if (!(fd in Sockets)) throw 'fd not a socket: ' + fd;
			...
			toRead      : throwTODO, 
			setNoDelay  : throwTODO,
			getpeername : throwTODO,
			socketpair	: throwTODO,
			pipe		: throwTODO,
			sendFD		: throwTODO,
...
function throwTODO(name){ throw new Error(' @@ TODO @@ ' + (name || '')) }
