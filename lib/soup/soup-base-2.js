...
//				if (fd == 8) debugger;
				...
				if (fd in Sockets) {
					...
					if (!handle.pair || !(handle.pair in Sockets)) {
						...
						defer(function(){ console.log(4444444, peer.readable, peer.fd);  peer.readable = true });
						// writable ??????
				...
				if (fd in Sockets){
					...
					if (how == 'write' || how == 'readwrite') {
						...
						if (!handle.shutdown.read){	// trigger reader
							var h = handle;
							defer(function(){ console.log(555555, h.readable, h.fd); h.readable = true });
						}
					}
			...
			read : read = function(fd, buffer, offset, length){
				...
				handle.readable = !reader.isEOF() || handle.shutdown.write;
				
				// mark its pair writable
				if (handle.pair && (handle.pair in Sockets)) { 
					var pair = Sockets[handle.pair];
//					pair.writable = true;// reader.canWrite();
				} 
...
//				handle.writable = false;
				...
				if (!handle.pair || !(handle.pair in Sockets)) return bytesWritten; //throw 'fd has no paired reader: ' + fd;

				var pair = Sockets[handle.pair];
				pair.readable = true; // ! writer.eof;
...
HTTPParser.prototype.execute = function(buffer, offset, length){
	...
	while (entity.length) {
		...
		if (chunked){
			...
			invoke('onBody', buffer, offset + bytesParsed, body.length);		// byte length or char length????????????
			bytesParsed += body.length;
		}
 ...
 */
var watchers = [];
var POLL_INTERVAL = 0;	// msec  above TICK_INTERVAL
...
//	timer.callback = poll;
	...
	function poll(){		
		...
		if (watchers.length) defer(poll);
		else timer = false;
		
//		watchers.forEach(function(w, next){			//////////////////// <<<<< or one at a time!!!!!
		...
		while (next < watchers.length) {
			...
			if (1) console.log(next, watchers.length,'--- watching w, fd, havedata: ',  fd
...
//				timer = setTimeout(poll, 0);	// at next tick!
//			timer.start(0,POLL_INTERVAL);
		}
		this.stop = function(){
			var index = -1,
				self = this;
...
Buffer.prototype.copy = function(target, targetStart, sourceStart, sourceEnd){
	...
	if (typeof sourceEnd == 'undefined') sourceEnd = this.length-1;
	sourceEnd += 1;
...
function ioBuffer(size){
	this.buffer = new Buffer(size || (4 * 1024) );	// size >= fs.ReadSteram.bufferSize
...
//	var eod = this.eod.shift() || 0;
	...
	var bytesRead = this.buffer.copy(buffer,offset,this.readhead, eod -1);
...
function timeWrap(method) {
	...
	return function(){ 
		...
		var arg = pSlice.call(arguments);
		
		arg[1] += TICK_INTERVAL;	// extend time 

//		console.log(':::::::::',arg);

		return original.apply(window, arg);
	}
}
