...
nodeSoup._hijackModule = function(){
	...
	if (Module.prototype._compile_orig ) debugger;
	...
	Module.prototype._compile = function(content, filename){
		...
		while (1) {
			...
			if (/.+\.js$/.test(line1)) {
				...
				var path = nodeSoup.loadModule('path'),
					fn = path.join(path.dirname(filename), line1);
				content = nodeSoup.loadModule('fs').readFileSync(fn, 'utf8');
			} else {
				break;
			}
		}
		
		this._compile_orig.call(this,content,filename);
...
process.compile = function(/*source, file*/){
	console.log(typeof process.mainModule);
