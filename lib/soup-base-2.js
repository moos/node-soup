...
process.pid = 200; 
process.installPrefix = process.env.NODE_PATH;
process.execPath = process.env.NODE_PATH + '/build/default/node'; 
...
process.compile = function(/*source, file*/){
	if (arguments[0].indexOf("::"+arguments[1]+"::") != -1){
		...
		var _content = arguments[0].replace("::"+arguments[1]+"::", window._getFileContent(process.env.NODE_PATH +'/lib/'+ arguments[1] + '.js') );
