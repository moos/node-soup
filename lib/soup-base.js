...
process.emit = function(type /* ,args */){
	...
	if (type == 'exit') {
		...
		if (waitEvents.length == 1){
			setTimeout(function(){ new process.IOWatcher().stop() },250); // a little time for things to settle
		}
...
function Buffer(arg, encoding) {
	...
	if (typeof arg == 'string') {
		var encodedStr = unescape(encodeURIComponent(arg));
...
Script.prototype.runInNewContext = function(sandbox){
	var result = nodeSoup.sandbox.eval(makeWritableScope(this.code), sandbox);
	return result;	
} 
Script.runInThisContext = function(code,filename){
	return eval(code); 
} 
Script.runInNewContext__WORKER__not_working = function(code, sandbox, filename){
	sandbox = sandbox || {};
	
	workerResult = null;
	workerDone = false;
	scriptWorker.postMessage({code:code, sandbox:sandbox});

	var its = 1000,
		timer = setTimeout(function(){ alert(workerDone); workerDone = 'hung' }, 10);	// 1.5 sec timeout
	
	// block!
//	var xhr = new XMLHttpRequest();
	while (! workerDone  && --its) {
		// twiddle dee
//		xhr.open('GET','./existential',false);
//		xhr.send();
	}
	clearTimeout(timer);
	if (its == 0) { //workerDone == 'hung') {
		//require('sys').debug('script timeout out');
		throw new Error('script timeout out');
	}
	// handle exception
	if ( typeof workerResult == 'object' &&
		'message' in workerResult && 'fileName' in workerResult && 'lineNumber' in workerResult) {
...
var workerDone = false,
	workerResult = null,
	scriptWorker = new Worker('../lib/worker.js');

scriptWorker.onmessage = function(event){

	console.info('msg from script worker ', event.data);
...
Script.runInNewContext = function(code, sandbox, filename){
	...
	var result = nodeSoup.sandbox.eval(code, sandbox);

	console.log('>>', code, sandbox, result);
	return result;
}
...
//  "parent.nodeSoup.sandbox=MSIE?this:{eval:function(s,c){with (c||{}){ return eval(s) } }}"+
...
process.compile = function(/*source, file*/){
	if (arguments[0].indexOf("::"+arguments[1]+"::") != -1){
		...
		var _content = arguments[0].replace("::"+arguments[1]+"::", _getFileContent(process.env.NODE_PATH + arguments[1] + '.js') );
...
(eval( _getFileContent(process.env.NODE_PATH + '../src/node.js') )) (process);
