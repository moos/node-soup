...
HTTPParser.prototype.execute = function(buffer, offset, length){
	...
	for (var i=0, l=headers.length; i<l; i++){
		...
		// other lines are headers, eg: "Content-Type: text/html"
		parts = splitOnFirst(line,':');

		// to simplify: not calling onHeaderField/Value since they take buffer
		...
		if (/Transfer-Encoding/i.test(parts[0]) && /chunked/i.test(parts[1]))
			chunked = true;
		
		// TODO handle multilline headers!!
	}
	
	// TODO handle footer-headers after chunked data 
 ...
 * dynamic size & grows as needed!  or IS IT??????
 */
...
process.compile = function(/*source, file*/){
	if (arguments[0].indexOf("::"+arguments[1]+"::") != -1){
		...
		// return eval(arguments[0].replace("::"+arguments[1]+"::", getFileContent(process.env.NODE_PATH + arguments[1] + '.js') ));
		
		// TEMP::: this format aids in debugging in Firebug!!!
		
		var _content = arguments[0].replace("::"+arguments[1]+"::", getFileContent(process.env.NODE_PATH + arguments[1] + '.js') );
...
(eval( getFileContent('../deps/node/src/node.js') )) (process);
